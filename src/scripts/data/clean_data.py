...
from src.data.load_data import load_raw_reddit_data
...
def clean_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data_to_change = data[data.columns.difference(exclude)]
    data_to_change = data_to_change.applymap(lambda s: s.lower() if type(s) == str else s)
    data[data_to_change.columns] = data_to_change

    data = data.replace("i don't boulder", pd.NA).replace("i don't climb routes", pd.NA)
    data["climbing_activities"] = data["climbing_activities"].apply(lambda x: x.lower()).astype("category")
    ...
    data["arm_span_cm"] = data["arm_span_cm"].replace({"5 ft 10 inches": 177.8, "0": np.nan})
...
def transform_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data["climbing_exp_years"] = data["climbing_exp_years"].apply(extract_years_range_and_average).astype(float)
    ...
    data["recent_max_climbed_grade_fb"] = data["recent_max_climbed_grade_v"].map(v_to_fb_mapping)
    data["consistent_grade_fb"] = data["consistent_grade_v"].map(v_to_fb_mapping)

    data["max_climbed_grade"] = data["max_climbed_grade_v"].map(v_to_numeric_mapping).astype(float)
    data["recent_max_climbed_grade"] = data["recent_max_climbed_grade_v"].map(v_to_numeric_mapping).astype(float)
    data["consistent_grade"] = data["consistent_grade_v"].map(v_to_numeric_mapping).astype(float)

    # Convert the route grades
    data["max_route_grade_french"] = data["max_route_grade_ewbank"].map(ewbank_aus_to_french_grade_mapping)
    data["inferred_max_grade_v"] = data["max_route_grade_french"].map(french_sport_to_v_grade_mapping)
    ...
    data["inferred_recent_grade_v"] = data["recent_max_route_grade_french"].map(french_sport_to_v_grade_mapping)
    data["inferred_recent_grade"] = data["inferred_recent_grade_v"].map(v_to_numeric_mapping)
    data["consistent_route_grade_french"] = data["consistent_route_grade_ewbank"].map(
        ewbank_aus_to_french_grade_mapping
    )
    data["inferred_consistent_grade_v"] = data["consistent_route_grade_french"].map(french_sport_to_v_grade_mapping)
    data["inferred_consistent_grade"] = data["inferred_consistent_grade_v"].map(v_to_numeric_mapping)

    data["hangboard_max_weight_half_crimp_kg"] = (
        data["hangboard_max_weight_half_crimp_kg"].apply(convert_weighted_hangboard_to_kg).astype(float)
    )
    data["hangboard_max_weight_open_crimp_kg"] = (
        data["hangboard_max_weight_open_crimp_kg"].apply(convert_weighted_hangboard_to_kg).astype(float)
    )
    ...
    data["max_weight_pull_ups_kg"] = data["max_weight_pull_ups_kg"].apply(convert_to_kg).astype(float)
    # manually fix outliers (as it seems that the values are in lbs + bw)
    data.loc[[60, 367, 419, 558, 576], "max_weight_pull_ups_kg"] = (
        data.loc[[60, 367, 419, 558, 576], "max_weight_pull_ups_kg"] - data.loc[[60, 367, 419, 558, 576], "weight_lbs"]
    ) * LBS_TO_KG

    data["max_pull_ups"] = pd.to_numeric(data["max_pull_ups"], errors="coerce")

    data["max_push_ups"] = data["max_push_ups"].apply(convert_max_push_ups).astype(float)

    data["max_l_sit_sec"] = data["max_l_sit_sec"].apply(convert_max_l_sit_sec).astype(float)

    # Add dummy columns
    data["is_male"] = data["gender"].apply(lambda x: 1 if x == "male" else 0)

    data["endurance_training_type"] = data["endurance_training_type"].fillna("").str.split(", ")
    data["endurance_training_type"] = data["endurance_training_type"].apply(
        lambda x: ["_".join(re.sub(r"[^a-zA-Z0-9]+", " ", y.lower()).split(" ")) for y in x]
    )
    endurance_dummies = pd.get_dummies(data["endurance_training_type"].explode(), prefix="endurance")
    endurance_dummies = endurance_dummies.groupby(endurance_dummies.index).agg(lambda x: x.max())
    ...
    grip_dummies.columns = ["grip_" + col.replace(" ", "_") for col in grip_dummies.columns]
    grip_dummies["grip_no_hangs"] = grip_dummies["grip_i_don't_hangboard"] + grip_dummies["grip_no_hangs"]
    ...
    style_dummies = data["hangboard_style"].replace({"i don't hangboard": "no_hangs"}).str.get_dummies(sep=", ")

    # Rename the columns for clarity
    style_dummies.columns = ["hangboard_style_" + col.replace(" ", "_") for col in style_dummies.columns]
    style_dummies["hangboard_style_no_hangs"] = (
        style_dummies['hangboard_style_"no_hangs"'] + style_dummies["hangboard_style_no_hangs"]
    )
    style_dummies.drop(columns=['hangboard_style_"no_hangs"'], inplace=True)
    style_dummies = style_dummies.astype(bool)

    data["strength_training_type"] = data["strength_training_type"].fillna("").str.split(", ")
    data["strength_training_type"] = data["strength_training_type"].apply(
        lambda x: ["_".join(re.sub(r"[^a-zA-Z0-9]+", " ", y.lower()).split(" ")) for y in x]
    )
    strength_dummies = pd.get_dummies(data["strength_training_type"].explode(), prefix="strength")
    strength_dummies = strength_dummies.groupby(strength_dummies.index).agg(lambda x: x.max())

    data = pd.concat([data, style_dummies, grip_dummies, endurance_dummies, strength_dummies], axis=1)
